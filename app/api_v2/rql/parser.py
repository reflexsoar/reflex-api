...
class QueryLexer(object):
    ...
    def t_target(self, t):
        ...
        #'''
        r'''observables(\.([\.\w]+))?|value|tlp|tags|spotted|safe|source_field|description|data_type|ioc|original_source_field|title|organization|severity|risk_score|status(\.([\.\w]+))?|reference|source|signature|tags|raw_log(\.([\.\w]+))?'''
        return t
